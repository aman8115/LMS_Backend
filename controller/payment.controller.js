...
const getRazorPayId = async (req,res,next)=>{
  res.status(200).json({
    success: true,
...
const buySubscription = async (req,res,next)=>{
 try{
  ...
  const subscription = await razorpay.subscriptions.create({
    plan_id:'plan_NYEBlFEZ7BYXxD',// The unique plan ID
...
console.log(subscription)
  ...
  res.status(200).json({
    ...
    subscription_id: subscription.id,
       ...
       return next(new AppError(" course not subscribe "),500)
 }
}
const verifyUserPayment= async(req,res,next)=>{
    ...
    //  const generatedSignature = crypto
     ...
     // }
      ...
      res.status(500).json({
        success:false,
        message:` payment not verify ${e.message}`
      })
    }
}
const cancelSubscription = async(req,res,next)=>{
   try{
     ...
     if(user.role == 'ADMIN'){
      return next ( new AppError(" Admin cannot cancel the course subscription"))
     }
     const subscriptionId = user.subscription.id
     console.log(" controller id ", subscriptionId)
     const subscription = await razorpay.subscriptions.cancel(subscriptionId)
     console.log("cancel controller subscription" , subscription)
     user.subscription.status = subscription.status;
     await user.save()
     res.status(200).json({
      success:true,
    ...
    return  next( new AppError(` subscription cannot cancel ${e.message}`))
